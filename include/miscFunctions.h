...
filesystem::path GetPathObject(std::initializer_list<string> input, bool interrupting = false);
string __CheckPathValidity(const filesystem::path& pathToCheck, bool interrupting, const string& failLocation); 
string GetPath(std::initializer_list<string> input = {}, bool interrupting = false);
string GetPathFromResources(std::initializer_list<string> input = {}, bool interrupting = false);
