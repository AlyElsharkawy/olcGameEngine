...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		if (GetKey(olc::Key::UP).bHeld)
			vCamera.y += 12.0f * fElapsedTime;	// Travel Upwards

		if (GetKey(olc::Key::DOWN).bHeld)
			vCamera.y -= 12.0f * fElapsedTime;	// Travel Downwards


		// Dont use these two in FPS mode, it is confusing :P
		if (GetKey(olc::Key::LEFT).bHeld)
			vCamera.x -= 12.0f * fElapsedTime;	// Travel Along X-Axis
		...
		vec3d vForward = Vector_Mul(vLookDir, 12.0f * fElapsedTime);
